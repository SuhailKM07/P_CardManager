...
export const CreditCardLayout = () => {
    ...
    if (pushNext && isBtnPressed) {
      setTimeout(() => {
        ...
        const StoreCustomerId = async () => {
          try {
            const storedValue = await AsyncStorage.getItem('cardNumber');
            const existingCards = storedValue ? JSON.parse(storedValue) : []; // Parse or default to empty array

            const updatedCards = [...existingCards, cardNo];
            ...
            console.log('Updated Stored Card Numbers:', updatedCards);
          } catch (error) {
  ...
  const [temp, setTemp] = useState(CardDetailsData);

  useEffect(() => {
    setTemp(CardDetailsData);
