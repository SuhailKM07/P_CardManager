...
import {FireBaseCardDetails} from '../../redux/slices/CardDetailsSlice';
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const fetchCards = async () => {
      try {
        let localCardNumbers = await AsyncStorage.getItem('cardNumber');
        const existingCards = localCardNumbers
          ? JSON.parse(localCardNumbers)
          : [];

        const allCards = await getAllUsers();
        if (!existingCards.length || !allCards?.length) return;

        const matchingCards = allCards.filter(card =>
          existingCards.includes(card?.cardNo),
        );
        dispatch(FireBaseCardDetails(matchingCards));
      } catch (error) {
        console.error('Error fetching Firebase cards:', error);
      }
    };
  ...
  const ChangingTheScreenHeight = useAnimatedStyle(() => ({
    ...
    bottom: withTiming(
      onPurchase.value && CardDetailsData?.length > 1 ? 0 : 'auto',
      {duration: 800},
    ),
  }));
